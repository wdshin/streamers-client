...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  const register = useCallback(
    (email: string, password: string) =>
      new Promise((resolve, reject) => {
        ...
        userPool.signUp(email, password, newAttributes, [], error => {
          if (error) {
            reject(error)
            console.error(error)
            return
  ...
  const verify = useCallback(
    (code: string) =>
      new Promise((resolve, reject) => {
        const userData = new CognitoUser({
          Username: window.localStorage.getItem('emailForRegistration') || '',
          Pool: userPool,
        })

        userData.confirmRegistration(code, true, error => {
          if (error) {
            reject(error)
            console.error(error)
            return
